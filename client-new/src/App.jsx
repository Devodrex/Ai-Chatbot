...
// ─── BACKEND CONFIG ───────────────────────────────────────────────────────────
// ✅ FIX 1 & 2: Use Vite env variable instead of process.env.REACT_APP_API_URL
...
export default function App() {
  ...
  // ── 1. Init: create session + load existing sessions ────────────────────
  useEffect(() => {
    const init = async () => {
      try {
  ...
  // ── 3. Send message ─────────────────────────────────────────────────────
  // ✅ FIX 5: No streaming/reader logic — backend returns { reply: "text" }
  const sendMessage = async (overrideMsg) => {
    ...
    setChat((prev) => [
      ...prev,
      { role: "user", text, ts },
      { role: "assistant", text: "", streaming: true, ts },  // ✅ FIX 3: "assistant" not "bot"
    ]);

    try {
      // ✅ FIX 4: Backend returns { reply: "..." } — use res.data.reply
      ...
      setChat((prev) => {
        ...
        updated[updated.length - 1] = {
          role: "assistant",   // ✅ FIX 3: always "assistant"
          text: reply,
          streaming: false,
          ts,
        };
  ...
  const loadSession = async (sess) => {
    ...
    try {
      ...
      const formatted = res.data.map((m) => ({
        role: m.role,           // ✅ FIX 3: backend should already return "user"/"assistant"
        text: m.content,
  ...
  const S = {
    root: {
      ...
      color: "#e2e8f0", position: "relative", boxSizing: "border-box",
    },
    leftNav: {
      width: 56, flexShrink: 0, display: "flex", flexDirection: "column",
      alignItems: "center", padding: "16px 0", gap: 6, zIndex: 10,
    ...
    infoPanel: {
      ...
      padding: "16px 12px", zIndex: 9, scrollbarWidth: "thin",
    },
    infoSection: {
      marginBottom: 20,
  ...
  return (
    <div style={S.root}>
      <style>{`*, *::before, *::after { box-sizing: border-box; } html, body, #root { margin: 0; padding: 0; width: 100%; height: 100%; overflow: hidden; }`}</style>
      <ParticleField />
      <ScanLine />
      ...
      <div style={S.mainArea}>
        ...
        <AnimatePresence>
          {backendStatus === "offline" && (
            ...
            >
              ⚠️ Cannot reach <code style={{ color: "#f87171" }}>{BASE_URL}</code> — start your backend server first.
            </motion.div>
          )}
        </AnimatePresence>
              ...
              <motion.div key={i}
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    {/* ✅ FIX 3: Check role === "user" not role === "bot" */}
        ...
        <div style={S.inputArea}>
          ...
          <div style={S.inputRow}>
            ...
            <textarea
              ...
              rows={1}
              placeholder={
                backendStatus === "offline"   ? "Backend offline — start server first…" :
          ...
          <motion.div style={S.infoPanel}
            ...
            <div style={S.infoSection}>
              ...
              <div style={S.statRow}>
                ...
                <span style={{ fontSize: 11, color: "#334155", fontFamily: "monospace" }}>{BASE_URL.replace("http://", "").replace("https://", "")}</span>
              </div>
              <div style={S.statRow}>
